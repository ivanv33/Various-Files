...
Given Goldratt's Bearington plant, where the NCX-10 and the heat-treat oven are the only stations that cannot keep up, the manager is ordered to keep cost per part down and cut lead time, and Jonah says only that an hour lost at the NCX-10 is an hour lost for the whole plant, applying Theory of Constraints and the Evaporating Cloud we derived that the conflict exists only because one batch size governs every station, that a setup hour is lost output only at the constraint, and that local efficiency stands in for plant cost; what the reader gains is the injection the scenario never states: large batches at the NCX-10 and heat treat, small everywhere else, meeting both orders.
