...
Given Pólya's classroom dialogue, in which a student seeking the diagonal of a rectangular parallelepiped names the unknown, lists the three edges as data, states that those edges alone fix the diagonal, offers the diagonal of a rectangle as a related problem, then guesses a plus b plus c, refuted by a unit square, applying Pólya's 4-Step Method we derived the plan the dialogue withholds (the base diagonal, Pythagoras twice), the answer, the root of a squared plus b squared plus c squared, and the checks it never runs (dimensions, c equals zero, the cube); the reader gains the empty fourth phase exposed: nobody looks back, and the transferable method, three dimensions reduced to two, is stated only here.
