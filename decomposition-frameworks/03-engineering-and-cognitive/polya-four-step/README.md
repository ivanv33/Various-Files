...
- `cd2` **Figure drawn, edges named a, b, c** [fact] The figure and the notation, which Polya counts as part of understanding the problem: a box is drawn, its three edges are marked a, b and c, and the diagonal is drawn from one corner to the corner furthest from it. "The student draws a box, marks the three edges a, b and c" (sentence 8)
...
The nineteen derived edges. One is the borrowing: `ceb1` `cpl1` -> `ca1` (borrows_from, label "same unknown, 2-D", 0.90), rationale "The plan is the related problem applied twice. Sentence 10 says explicitly that the student cannot see the link, so the link is the inference." Three hang the steps on the plan: `cee1` `cpl1` -> `cs1` (executes, 0.92), `cee2` `cpl1` -> `cs2` (0.90), `cee3` `cpl1` -> `cs3` (0.90). Seven are `uses`: `ceu0` `cd2` -> `cd1` (0.85, the edges marked a, b, c are the three given lengths), `ceu1` `cs0` -> `cd1` (0.80, the guess consumes the givens), `ceu2` `cs1` -> `cd2` (0.90), `ceu3` `cs2` -> `cd2` (0.88), `ceu4` `cs2` -> `cs1` (0.92), `ceu5` `cs3` -> `cs2` (0.92), `ceu6` `ck2` -> `ca1` (0.80, the c = 0 case consumes the related problem as a known answer). Two verify the result: `cev1` `ck1` -> `cs3` (0.85), `cev2` `ck2` -> `cs3` (0.88). Two generalize: `ceg1` `cg1` -> `cpl1` (0.85, "names what the plan did in a form that does not mention boxes"), `ceg2` `cg2` -> `cs3` (0.80, "extends the result, not the method"). Four are grounding links: `cesb1` `cs3` -> `cu1` (0.90), `cesb2` `ck1` -> `cd1` (0.80), `cesb3` `cg1` -> `ca1` (0.85), `cesb4` `cg2` -> `cd1` (0.80).
...
- `td2` **About $50B per gigawatt, recalled** [fact] A second per-gigawatt price, recalled by the researcher from an analyst: around fifty billion dollars per gigawatt. The attribution is unintelligible in the transcript and the number is never sourced on air. "he said it's like around 50 billion per gigawatt" (Tyler, L1694)
- `td3` **1 GW of MI-450 starts H2 next year** [fact] The ramp: OpenAI will begin with one gigawatt worth of MI-450 chips in the second half of next year, so the six gigawatts arrives in stages. Entities: OpenAI, AMD MI-450. "open AI is going to begin with one gigawatt worth of MI 450 chips starting in the second half of next year to run its AI models." (host, L1600-L1602)
...
PDCA / Deming Cycle, in Sensemaking & Complex Systems, is the closest relative not linked here: the same four phases applied to a process rather than a problem, with Act in the part of Look back — skipped for the same reason.
