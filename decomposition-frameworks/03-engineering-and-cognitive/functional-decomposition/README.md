...
Given a structured-analysis scenario in which Northwind Foods' four-step payroll batch (validation, gross pay, deductions, disbursement) was reordered to save twenty minutes and paid three hundred people the right gross and the wrong tax while the ledger stopped balancing against the bank file, applying Functional Decomposition we derived the promises the specification never wrote: gross pay is final before deductions read it, validated shifts match one employee and overlap none, the bank file and the journal agree to the penny, and a fifth, unnamed function owns the order the steps run in; what the reader gains is the exposed assumption that the one stated rule, each step reads only what the step before it wrote, governed access, not completeness.
