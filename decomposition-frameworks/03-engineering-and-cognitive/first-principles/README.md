...
Seventeen of the twenty-five nodes and six of the thirty-four edges are facts, none of them paraphrases. `source_ref` is the speaker plus the line range in the file.
...
Fact edges. Six, each joining two fact nodes inside a single turn that states the connection.
...
- `te4` `t3` -> `t4` (implies) [fact] "The silicon is enormously abundant on the Earth. We've gotten really quite good at making it. There are factories worldwide now churning out more than a terawatt of solar per year" (Casey Handmer, L3272-L3278)
...
Eight derived nodes and twenty-eight derived edges. Fact nodes are referenced by id.
