...
- [ ] **Step 1: Launch in batches of at most 6.** The harness allows 20 concurrent subagents (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`), and each framework parent holds 2 to 3 slots because its children now run blocking, so about 6 frameworks saturate the pool. Launching more returns "Concurrent subagent limit reached" and the extra launches are simply lost. Wait for a batch to drain before launching the next. Order: 02-strategic-and-business (mece, issue-hypothesis-trees, five-whys, ishikawa-fishbone, rumelt-strategy-kernel, theory-of-constraints), then 03-engineering-and-cognitive (first-principles, functional-decomposition, polya-four-step, means-ends-analysis, inversion-premortem), then 04-sensemaking-and-complex-systems (cynefin, wardley-mapping, ooda-loop, pdca-deming, systems-thinking).
