...
Given the dispute in which the rationalists, Descartes, Spinoza and Leibniz, held that knowledge is founded in innate reason, citing mathematics and the deceit of the senses, and the empiricists, Locke, Berkeley and Hume, held that every idea copies an impression, citing the never-exhibited innate idea and necessity as habit, applying Dialectical Decomposition we derived that the surface contest is over a single source, that both sides share an unexamined assumption of one source and a passive mind, and that Kant's synthesis lets the mind supply form and experience supply content; the opportunity that falls out is the hidden assumption exposed, since necessity then belongs to the form the mind imposes and Hume's objection is answered rather than split with.
