...
Given a note left on Tuesday morning, written in the order the calls came in (John Collins cannot make three o'clock because he is on a train until six, Hal Johnson cannot start before half past ten because of a daily board call, Don Clifford cannot come before Thursday, and the conference room is booked tomorrow but free on Thursday), applying the Pyramid Principle we derived what the note never writes: today is impossible, tomorrow is impossible, so Thursday is the first day all three can meet, and eleven is the earliest hour that clears Johnson's constraint. What the reader gains is the intersection the writer left them to compute, and the answer lifted from last line to first.
